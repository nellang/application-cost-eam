...
export class ApplicationCostComponent implements OnInit {
  ...
  fbDeleteCost() {
    this.tbdeleted.forEach(cost => {
      this.root.child('/ECost/' + cost.name).remove();
    });
      ...
      if (snapshot.hasChild('/ECost/')) {
        this.root.child('/ECost/').once('value', (childSnapshot) => {
          childSnapshot.forEach((costSnapshot) => {
            ...
            this.tbdeleted.push(createApplicationCost(cost.AName, cost.BDescription, costEntries));
          });
        });
      }
